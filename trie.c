...
TrieNode* findNode(TrieNode* n, char c){
...
/*
Try to parse a word or symbol that is delimited:
forif will parse as id[forif] and not word[for] word[if]
but
<< will parse as sym[<<] and not sym[<], sym[<]
Words don't matter to the trie? It just tried to parse the biggest identifier possible.
Symbols do
*/